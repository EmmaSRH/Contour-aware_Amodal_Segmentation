...
class EndovisDataset(utils.Dataset):
    ...
    def load_balloon(self, dataset_dir, subset):
        ...
        with open(dataset_dir, 'r') as f:
            ...
            for line in lines:
                str_list = line.split(' ')
                imgs.append(str_list[0].replace('home','data'))
        print('************Train Image Num is: ',len(imgs),'***********')
        i = 0
        for img in imgs:
            # Add images
            image_path = img.replace('home','data')
            image = skimage.io.imread(image_path)
            height, width = image.shape[:2]
            # print(height,width)

            mask_o = skimage.io.imread(img.replace('raw', 'instrument_instances'))
            # print(mask_o.shape)
            polygons = np.max(mask_o)
            # print(polygons)
...
def train(model):
    ...
    dataset_val.load_balloon(val_data, "val")
    dataset_val.prepare()
