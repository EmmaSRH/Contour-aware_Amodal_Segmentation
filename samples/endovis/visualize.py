...
def display_instances(image, pres, masks, class_ids, class_names,
                      scores=None, title="",
...
def display_differences(image,pred_masks,gt_masks,
    ...
    gt_match, pred_match, overlaps = compute_matches(pred_masks, gt_masks)
...
def transparent_back(imgs):
    for img in imgs:
        img = img.convert('RGBA')
        L, H = img.size
        color_0 = img.getpixel((255,255))
        for h in range(H):
            for l in range(L):
                dot = (l,h)
...
if __name__ == '__main__':
    ...
    for mode in modes:
        ...
        for img in list:
            ...
            display_differences(image,pre_list,gt_list, pred_score, pred_mask, class_names,)

            image = Image.open(img)
            pres = []
            for pre in pre_list:
                pres.append(Image.open(pre))
            new_pres = transparent_back(pres)
            for pre in new_pres:
                img = img+new_pres
            img.save('new.png')
